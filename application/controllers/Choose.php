...
class Choose extends CI_Controller
{
    ...
    ////////////////////end of excel library option
        ...
        public function register(){
            ...
            if($this->input->post('numbers'))
            {
                $data['id']=$this->input->post('numbers');
                $this->load->view('admin/reports/get_id_number',$data);
            }elseif($this->input->post('add')){
                $this->Register->insert($id);
                $this->message('success','تم ارسال ');
                redirect('Web','refresh');
            }else{
                $data['subview'] = 'web/register';
                $this->load->view('index1', $data);
            }
